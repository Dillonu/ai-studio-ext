 ...
 */
function createAuthParams(url, token, additionalParams) {
    ...
    // Complex case with additionalParams as array of objects with key/value pairs
        ...
        const baseParams =
            values.length === 0 ? [timestamp, token, url] : [values.join(":"), timestamp, token, url];
 ...
 */
function createAuthHash(token, hashName, additionalParams) {
 ...
 * @returns {string|null} Authentication hash or null
 */
function getTokenAndCreateHash(windowProperty, cookieName, hashName, additionalParams) {
    // Try to get token from window property
    let token = window[windowProperty];
 ...
 */
export function getAuthTokens(authParams) {
    ...
    let hasToken =
        window.__SAPISID || window.__APISID || window.__3PSAPISID || window.__1PSAPISID || window.__OVERRIDE_SID;
    ...
    if (hasToken) {
        ...
        let token = isSecureProtocol ? window.__SAPISID : window.__APISID;
        ...
        if (isSecureProtocol) {
            // Get SAPISID1PHASH
            const secureToken1 = getTokenAndCreateHash(
                "__1PSAPISID",
                "__Secure-1PAPISID",
                "SAPISID1PHASH",
                authParams
            ...
            // Get SAPISID3PHASH
            const secureToken3 = getTokenAndCreateHash(
                "__3PSAPISID",
                "__Secure-3PAPISID",
                "SAPISID3PHASH",
                authParams
...
function createHashGenerator() {
     ...
     */
    function update(data, dataLength) {
    ...
    function digest() {
        ...
        for (let i = 63; i >= 56; i--) {
            ...
            bitLength >>>= 8; // Fixed: missing assignment operator
        }
